...
function muatLogo(mediaId) {
  fetch(API_ROOT + "wp/v2/media/" + mediaId)
    ...
    .catch(function (err) {
      console.warn("Logo pakai ikon fallback:", err.message);
    });
}
...
function muatIdentitasSitus() {
  fetch(API_ROOT, { headers: { Accept: "application/json" } })
    .then(function (res) {
      if (!res.ok)
        throw new Error("Gagal memuat identitas situs (" + res.status + ")");
      return res.json();
    })
    .then(function (data) {
      if (data.name) {
...
function ambilFeaturedImage(halaman) {
  try {
    var media =
      halaman._embedded &&
      halaman._embedded["wp:featuredmedia"] &&
      halaman._embedded["wp:featuredmedia"][0];
...
function kartuBeritaHTML(halaman, gambarUrl, namaTag) {
  var judul = decodeEntities(
    (halaman.title && halaman.title.rendered) || "(Tanpa judul)",
  );
  var excerpt = truncate(
    stripHtml((halaman.excerpt && halaman.excerpt.rendered) || ""),
    130,
  );
  ...
  if (namaTag) {
    ...
    badgeHtml =
      '<span class="badge-tag" style="background-color: ' +
      styleW.bg +
      "; color: " +
      styleW.text +
      '; padding: 3px 8px; border-radius: 12px; font-size: 10.5px; font-weight: 600; display: inline-block; margin-bottom: 6px;">' +
      namaTag +
      "</span>";
  }
  ...
  return (
    ...
    link +
    '" target="_blank" rel="noopener">Baca selengkapnya ' +
    ARROW_SVG +
    "</a>" +
    "</div>" +
    "</article>"
...
function galeriKartuHTML(url) {
  return (
    '<div class="galeri-item"><img src="' +
    url +
    '" alt="Dokumentasi kegiatan PCNU Kota Pasuruan" loading="lazy" draggable="false"></div>'
  );
}
...
function fetchGambarDenganTanggalDariMediaLibrary(query) {
  return fetch(API_ROOT + "wp/v2/media?" + query)
    .then(function (res) {
      return res.ok ? res.json() : [];
    })
      ...
      return (Array.isArray(daftarMedia) ? daftarMedia : [])
        ...
        .map(function (m) {
          ...
          var url =
            ...
            (sizes && sizes.large && sizes.large.source_url) ||
            m.source_url;
          return url ? { url: url, tanggal: new Date(m.date) } : null;
        })
        .filter(Boolean);
    })
    .catch(function () {
...
function muatDaftarBerpaginasi(opsi) {
  ...
  var pagWrap = opsi.paginationId
    ? document.getElementById(opsi.paginationId)
    : null;
  if (!grid) return;
  ...
  function bangunPaginasi(totalPages) {
    ...
    html +=
      ...
      (halamanSekarang + 1) +
      '" ' +
      (halamanSekarang >= totalPages ? "disabled" : "") +
    ...
    ambilTagJikaPerlu()
      ...
      .then(function (hasil) {
        ...
        if (!Array.isArray(daftarItem) || daftarItem.length === 0) {
          tampilkanErrorBerita(grid, opsi.pesanKosong, function () {
            render(1);
          });
          return;
        }
